...
func (i *InspectCmd) Run(ctx context.Context) error {
...
func (i *InspectCmd) inspectPKToken(pktStr string) {
	...
	i.printf("\n--- ID Token Payload ---\n")
	var idTokenInfo struct {
		OpToken string `json:"op_token"`
	}
	if err := json.Unmarshal(pkt.Payload, &idTokenInfo); err != nil {
		i.printf("Error extracting ID token: %v\n", err)
	} else {
		parts := strings.Split(idTokenInfo.OpToken, ".")
		if len(parts) >= 2 {
			payload, err := base64URLDecode(parts[1])
			if err != nil {
				i.printf("Error decoding ID token payload: %v\n", err)
			} else {
				i.printJSON(payload)
			}
		}
