...
func (l *LoginCmd) login(ctx context.Context, provider providers.OpenIdProvider, printIdToken bool, seckeyPath string) (*LoginCmd, error) {
	...
	certBytes, seckeySshPem, err := createSSHCertWithAccessToken(pkt, accessToken, signer, l.PrincipalsArg, certValidBefore(pkt, l.AutoRefreshArg))
	if err != nil {
	...
	// Additionally load the certificate into a running ssh-agent (if one is
	// reachable via SSH_AUTH_SOCK), with a lifetime matching the ID Token's exp
	// so the agent drops it when the certificate expires. Non-fatal: a failure
	// here must not fail the login, since the keys were already written above.
	if !l.PrintKeyArg {
		l.addCertToAgent(certBytes, signer, pkt)
	}
...
func (l *LoginCmd) LoginWithRefresh(ctx context.Context, provider providers.RefreshableOpenIdProvider, printIdToken bool, seckeyPath string) error {
	if loginResult, err := l.login(ctx, provider, printIdToken, seckeyPath); err != nil {
		...
		for {
			...
			// --auto-refresh: the cert is re-minted on every refresh and must
			// stay valid across token refreshes, so it does not expire with any
			// single token.
			certBytes, seckeySshPem, err := createSSHCertWithAccessToken(loginResult.pkt, accessToken, loginResult.signer, loginResult.PrincipalsArg, ssh.CertTimeInfinity)
			if err != nil {
...
func createSSHCert(pkt *pktoken.PKToken, signer crypto.Signer, principals []string) ([]byte, []byte, error) {
	return createSSHCertWithAccessToken(pkt, nil, signer, principals, certValidBefore(pkt, false))
}

// certValidBefore returns the SSH certificate's ValidBefore for a login. For a
// one-shot login it is the ID token's exp, so the certificate cannot outlive the
// token whose authorization it carries. For --auto-refresh it is
// ssh.CertTimeInfinity: the refresher re-mints the cert in place as the token is
// refreshed, and the cert must remain valid across those refreshes. Falls back
// to CertTimeInfinity when the token carries no usable exp.
func certValidBefore(pkt *pktoken.PKToken, autoRefresh bool) uint64 {
	if autoRefresh {
	...
	if err := json.Unmarshal(pkt.Payload, &claims); err != nil || claims.Expiration <= 0 {
		return ssh.CertTimeInfinity
	}
	return uint64(claims.Expiration)
}

// addCertToAgent loads the freshly minted certificate and its private key into
// the ssh-agent reachable via SSH_AUTH_SOCK, with a lifetime bounded by the ID
// Token's exp so the agent drops it when the certificate expires. It is
// best-effort: any problem is reported and swallowed so login still succeeds
// (the keys were already written to disk before this is called).
func (l *LoginCmd) addCertToAgent(certBytes []byte, signer crypto.Signer, pkt *pktoken.PKToken) {
...
func createSSHCertWithAccessToken(pkt *pktoken.PKToken, accessToken []byte, signer crypto.Signer, principals []string, validBefore uint64) ([]byte, []byte, error) {
	cert, err := sshcert.New(pkt, accessToken, principals, validBefore)
	if err != nil {
		return nil, nil, err
	}
