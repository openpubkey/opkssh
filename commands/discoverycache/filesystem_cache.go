...
// Invalidate removes all cache entries for the given issuer
func (c *FilesystemDiscoveryCache) Invalidate(ctx context.Context, issuer string) error {
	afs := afero.Afero{Fs: c.Fs}
	chErr := make(chan error)
	go func() {
		issuerHash := fmt.Sprintf("%x", sha256.Sum256([]byte(issuer)))
		targetDir := filepath.Join(c.BaseDir, issuerHash, "jwks")
		if err := afs.RemoveAll(targetDir); err != nil {
			chErr <- err
		}
		close(chErr)
	}()
