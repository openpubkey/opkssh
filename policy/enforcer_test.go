...
func TestPolicyWildcardEmailAtBoundary(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:              "suffix without @ does not match lookalike domain",
			identityAttribute: "oidc-match-end:email:example.com",
			email:             "attacker@evil-example.com",
			shouldMatch:       false,
		},
		{
			name:              "suffix without @ does not match exact domain",
			identityAttribute: "oidc-match-end:email:example.com",
			email:             "user@example.com",
			shouldMatch:       false,
		},
		{
			name:              "suffix with @ still matches",
			identityAttribute: "oidc-match-end:email:@example.com",
			email:             "user@example.com",
			shouldMatch:       true,
		},
		{
			name:              "suffix with @ does not match lookalike domain",
			identityAttribute: "oidc-match-end:email:@example.com",
			email:             "attacker@evil-example.com",
			shouldMatch:       false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			op, _, err := NewMockOpenIdProvider2(false, "https://accounts.example.com", "test_client_wildcard_bound", map[string]any{"email": tt.email})
			require.NoError(t, err)
			...
			err = policyEnforcer.CheckPolicy("test", pkt, "", "example-base64Cert", "ssh-rsa", policy.DenyList{}, nil)
			if tt.shouldMatch {
				require.NoError(t, err, "expected %q to match email %q", tt.identityAttribute, tt.email)
			} else {
				require.Error(t, err, "expected %q to not match email %q", tt.identityAttribute, tt.email)
			}
		})
	}
}
