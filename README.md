...
Tell opkssh to name the key-pair `opkssh_server_group1`
...
Via the `key_management` settings you can specify a default location `default_key_dir` for the SSH keypairs generated by opkssh.
Paths are always relative to your home directory.
...
Most SSH servers implement a limit of 6 authentication tries.
This means that after attempting to offer 6 ssh keys to a server it will close the connection with an auth fail.
opkssh will check how many providers are configured and output a warning if there are more than 6, assuming each has added a key pair.

It might be necessary to remove old keys if you use many provider and clientID combinations or if they change often.

When removing keys make sure to remove their `IdentityFile` entry in `~/.opk/config`, too.

#### Workaround

You can move the include directive for opkssh's config in `~/.ssh/config` to the end of your config file.
This will allow includes and keys declared before it to be attempted first.

Consider using host specific configurations:
```
Host <host alias>
