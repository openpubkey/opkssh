...
`,
  ...
  opkssh add developer oidc:groups:developer https://accounts.google.com`,
	...
	inspectCmd := &cobra.Command{
		SilenceUsage: true,
		Use:          "inspect [pub_key_certificate]",
		Short:        "Inspect and view details of an opkssh generated SSH public key",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			keyOrCert := args[0]
			inspect := commands.NewInspectCmd(keyOrCert, cmd.OutOrStdout())
			if err := inspect.Run(ctx); err != nil {
