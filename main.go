...
`,
  ...
  opkssh login --provider=<issuer>,<client_id>,<client_secret>,<scopes>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			login := commands.NewLogin(autoRefreshArg, configPathArg, createConfigArg, configureArg, logDirArg, sendAccessTokenArg, disableBrowserOpenArg, printIdTokenArg, providerArg, keyAsOutputArg, keyPathArg, providerAliasArg, keyTypeArg)
	...
	loginCmd.Flags().BoolVarP(&keyAsOutputArg, "private-key-output", "o", false, "Print private key and SSH cert instead of writing them to the filesystem")
