...
This should take you to this first step. Please note that the Client ID defined here will be used for opkssh client and server configurations (`opkssh` here).
...
Click "Next" and configure the following three redirect URLs:
```
...
### 2. Optional: add user's groups to the profile information

Include users' groups in their profile information if you want to allow them to connect to servers based on group membership rather than only their identity.
...
### 3. Update provider configuration on server and client
...
```
https://your-keycloak-instance.tld/realms/your-realm
```

To test, run `opkssh login --provider="https://your-keycloak-instance.tld/realms/your-realm,CLIENT_ID"` with the client ID you registered (`opkssh` in this guide).
...
It may be the case that your Keycloak instance and your machine are out of sync. By default, Keycloak issues ID tokens that are only valid for a short period of time.
